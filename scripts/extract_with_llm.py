...
class FinancialMetric(BaseModel):
    """Individual financial metric with dynamic year values and change percentage"""

    metric: str = Field(description="Name of the financial metric")
    change_percent: Optional[float] = Field(
        description="Percentage change from previous year", default=None
    )

    # Use model_config instead of Config class for Pydantic v2
    model_config = {"extra": "allow"}  # Allow additional fields for dynamic year values

    def get_year_value(self, year: int) -> Optional[float]:
        """Helper method to get value for a specific year"""
        field_name = f"value_{year}"
        return getattr(self, field_name, None)

    def set_year_value(self, year: int, value: Optional[float]):
        """Helper method to set value for a specific year"""
        field_name = f"value_{year}"
